...
//--------------------------------------------------------------------------------------------------------------------
struct Sudoku_level_2 : public testing::Test {
   public:
...
bool Sudoku_level_2::storageLoaded = false;
HMODULE Sudoku_level_2::hLibrary= NULL;
User* Sudoku_level_2::user= NULL;
Board* Sudoku_level_2::board = NULL;

TEST_F(Sudoku_level_2, output_sudoku) {
...
TEST_F(Sudoku_level_2, verifity_sudoku_statusT) {
   
    std::map<int,int> map_value ={{0,0},{2,0}};
...
TEST_F(Sudoku_level_2, verifity_sudoku_statusF) {
   
    std::map<int,int> map_value ={{4,0},{0,4}, {1,3} };
...
TEST_F(Sudoku_level_2, adding_velue_sudoku) {
   
    if (storageLoaded)
        ASSERT_TRUE(board->adding_velue(0,0,2));
  
}

TEST_F(Sudoku_level_2, verifity_table_sudoku) {
    std::string table_values = "238965711759413682416278953945136278687524139321897465162359847574681391893741526";
